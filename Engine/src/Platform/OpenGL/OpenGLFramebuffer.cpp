...
namespace Engine::OpenGL {
	...
	Ref<GlTexture2D> GlFramebuffer::GetColorAttachment(uint32_t attachment) const
	{
		if (attachment == 0)
			return m_ColorAttachment;

		if (attachment == 1)
			return m_ColorAttachment1;
	...
	void GlFramebuffer::Init()
	{
		...
		if (m_Specification.ColorAttachment)
		{
			...
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_ColorAttachment->m_TextureID, 0);
		}
		else
		{
		...
		if (m_Specification.ColorAttachment1)
		{
			Texture2DSpecification colorSpec;
			colorSpec.Width = m_Specification.Width;
			colorSpec.Height = m_Specification.Height;

			colorSpec.Internalformat = m_Specification.ColorAttachment1.Internalformat;
			colorSpec.Format = m_Specification.ColorAttachment1.Format;
			colorSpec.Type = m_Specification.ColorAttachment1.Type;

			colorSpec.Wrap_S = m_Specification.ColorAttachment1.Wrap_S;
			colorSpec.Wrap_T = m_Specification.ColorAttachment1.Wrap_T;
			colorSpec.Border = m_Specification.ColorAttachment1.Border;

			colorSpec.Min_Filter = m_Specification.ColorAttachment1.Min_Filter;
			colorSpec.Mag_Filter = m_Specification.ColorAttachment1.Mag_Filter;

			colorSpec.Mipmaps = false;

			colorSpec.ComparisonMode = m_Specification.ColorAttachment1.ComparisonMode;

			const std::string name = m_Specification.ColorAttachment1.Name != std::string() ? m_Specification.ColorAttachment1.Name : "ColorAttachment1";
			m_ColorAttachment1 = CreateRef<GlTexture2D>(name, colorSpec);
			if (m_Specification.ColorAttachment1.TextureLibrary)
				Texture2DLibrary::AddSystem(m_ColorAttachment1);

			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, m_ColorAttachment1->m_TextureID, 0);
